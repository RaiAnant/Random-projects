...
int main(){
     ...
     while(cpu_process!=NULL||arrival_q_start!=NULL||ready_q_start!=NULL||io_q_start!=NULL){
          time_dur=slice_time*slice_no-curr_time;
          mode=0;
          printf("Scheduling........\n");
          if(cpu_process==NULL)
          printf("cpu sitting idle...");
          ...
          }else if(time_dur==slice_time*slice_no-curr_time){
               insert_ready_q(&ready_q_start,&ready_q_end,cpu_process);
               cpu_process=get_process(&ready_q_start,&ready_q_end);
          }

          if(time_dur==slice_time*slice_no-curr_time){
               slice_no++;
               printf("one time slice completed\n");
          }
